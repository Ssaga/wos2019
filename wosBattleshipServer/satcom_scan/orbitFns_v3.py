...
def satlla_from_TLE(line1, line2, verbose=False):
    ...
    numSteps = int(propTimeSecs/timeDivisor)
    #    STOPTIME = STARTTIME + datetime.timedelta(seconds = propTimeSecs)
    
    r = np.zeros((numSteps, 3))
    v = np.zeros((numSteps, 3))
    lat = np.zeros((numSteps, 1))
    lon = np.zeros((numSteps, 1))
    
    cartRepLog = []
    #print(STARTTIME)
    #print(STOPTIME)
    
    for i in range(numSteps): # propagate orbit and convert to LLA
        ...
        cartRepLog += [(cartRep, CURRTIME)]
    
    for i in [0, numSteps - 1]:
        lat[i], lon[i] = latlon_from_cartrep(*cartRepLog[i])
        if verbose:print(i, lon[i])
    exp_ao = 103.5
    
    gradMag = (360 - abs(lon[0] - lon[numSteps - 1])) / numSteps
    if verbose:print('gradMag', gradMag)
    exp_ao_lon_if_lon_increase = int((exp_ao - (lon[0] - 360*(lon[0] > exp_ao))) / gradMag)
    if verbose:print('exp_ao_lon_if_lon_increase', exp_ao_lon_if_lon_increase)
    exp_ao_lon_if_lon_decrease = int(((lon[0] + 360*(lon[0] < exp_ao)) - exp_ao) / gradMag)
    if verbose:print('exp_ao_lon_if_lon_decrease',exp_ao_lon_if_lon_decrease)
    for i in [exp_ao_lon_if_lon_increase, exp_ao_lon_if_lon_decrease]:
        lat[i], lon[i] = latlon_from_cartrep(*cartRepLog[i])
        if verbose:print(i, lon[i])
    if abs(lon[exp_ao_lon_if_lon_increase] - exp_ao) < abs(lon[exp_ao_lon_if_lon_decrease] - exp_ao):
        if verbose:print('lon is increasing')
        if lon[exp_ao_lon_if_lon_increase] > 102:
            i = exp_ao_lon_if_lon_increase
            while True:
                i -= 1
                lat[i], lon[i] = latlon_from_cartrep(*cartRepLog[i])
                if lon[i] < 102:break
        ...
        if verbose:print('lon is decreasing')
        if lon[exp_ao_lon_if_lon_decrease] < 105:
            i = exp_ao_lon_if_lon_decrease
            while True:
                i -= 1
                lat[i], lon[i] = latlon_from_cartrep(*cartRepLog[i])
                if lon[i] > 105:break
...
###########################################
def sar_boresight(r, lookangle, i, verbose=False):
    ...
    if verbose: print(np.rad2deg(theta))
